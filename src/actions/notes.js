...
export function getAllNotes(dummyAuth) {
  return async dispatch => {
    const res = await service.getallnotes(dummyAuth);
    if (res.data.status) {
      res.data.notes && (await dispatch(setAllNotes([...res.data.notes])));
    } else {
      await dispatch(setAllNotes([]));
      await dispatch(setNoteError(res.data.message));
    }
  };
}
