...
const Card = ({ note, onEdit, onReadMore }) => {
  return (
    <Fragment>
      <div className='card'>
        ...
        <p className='description'>{`${
          note.get('note').length > 150
            ? note.get('note').substring(0, 150) + '...'
            : note.get('note')
        }`}</p>
        <div className='card-meta'>
          <span className='timestamp'>{note.get('time')}</span>
          <a className='' onClick={() => onEdit(note)}>
            Edit
          </a>
          <a className='' onClick={() => onReadMore(note)}>
            Read More
          </a>
        </div>
      </div>
